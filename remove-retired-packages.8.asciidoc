...
Script *remove-retired-packages* checks which packages has been retired (removed) between current version of Fedora Linux and VERSION's version. When no parameters is passed then VERSION is set to current version minus one.
